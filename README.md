...
**29 tools**, spanning the full schema-authoring and enrichment surface:
...
| Schemas | `generate_sample` | Generate a realistic sample entity JSON from an entity-type description — the natural first step before create_schema_from_sample. |
...
| Schemas | `create_schema_from_sample` | Generate a new JSON schema from a sample entity dict, using an LLM to infer field types, key properties, and expertise grouping. |
| Schemas | `edit_schema` | Modify an existing saved schema using a natural-language prompt (e.g. 'add a regulatory_status field marked as a key property', 'remove the founding_year field', 'mark name as… |
...
| Enrichment & fusion | `fetch_entities` | Fetch a JSON array of entities from an external REST API (GET), server-side. |
...
| Enrichment & fusion | `merge_records` | Merge 2+ enrichment records of the same entity into one fused result. |
| Job control | `get_job_status` | Poll the status of an asynchronous LLM job (batch enrichment, sample generation, benchmark run, retry-expertises, ...). |
| Job control | `cancel_job` | Cancel a pending, running, or paused LLM job. |
| Job control | `answer_job_question` | Answer the clarification questions of a paused job and resume it (used by generate_sample's interactive planner). |
...
| Benchmarks | `get_benchmark_scenario` | Fetch one benchmark scenario with its per-model results (quality / cost / speed scores; results whose config_hash differs from the scenario's are stale). |
...
| Benchmarks | `set_benchmark_reference` | Save a scenario's gold reference — the expected enrichment output each model result is scored against. |
| Benchmarks | `delete_benchmark_scenario` | Delete a benchmark scenario and its results. |
| Benchmarks | `run_benchmark` | Launch a benchmark run: enrich the scenario's entity with each selected model (repetitions per model as configured), auto-score every success against the verified gold reference,… |
| Attachments | `upload_attachment` | Upload a file (base64-encoded) so it can be used as source material in an enrichment. |
