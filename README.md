...
**39 tools**, spanning the full schema-authoring and enrichment surface:
...
| Database Sync | `classify_database_model` | Re-run the database-model classification pass on a saved schema: an LLM proposes each property's SQL contract — database_key (identity), db_type (+length), indexed, and owned on… |
