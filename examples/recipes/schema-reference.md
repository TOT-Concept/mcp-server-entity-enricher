...
`analyze_schema` writes ambiguity and identity-scoping annotations; it does not perform suggested structural edits. Use a precise description to resolve ambiguous meaning on an established contract. A rename is possible through the property tools, but can require a migration. `resolve_unify_proposal` previews accepting or dismissing a cross-site entity-type proposal: acceptance combines sites under a winning definition and may make fields nullable. `nest_schema_region` materializes a logical entity region of `x-entityMap`. Flat members become a subobject: the region's fields and every region hanging from it move in, pair facts stay, and prefixes are shed (`product_name` → `product.name`). Compact scalar occurrences (`manufacturer_name` or each `therapeutic_classes[]` item) are all converted at once to refs to one shared `$defs` entity. Review the returned document before persisting.
